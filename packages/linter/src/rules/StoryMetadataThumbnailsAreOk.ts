...
export class StoryMetadataThumbnailsAreOk extends Rule {
  async run(context: Context) {
    ...
    function isAtLeast80x80({ width, height }: ISizeCalculationResult) {
      width = width ?? 0;
      height = height ?? 0;
    ...
    function isAtLeast480x640({ width, height }: ISizeCalculationResult) {
      width = width ?? 0;
      height = height ?? 0;
      return width >= 480 && height >= 640;
    }
    function isAtLeast640x480({ width, height }: ISizeCalculationResult) {
      width = width ?? 0;
      height = height ?? 0;
